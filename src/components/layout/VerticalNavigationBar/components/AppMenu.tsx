...
const AppMenu = ({ menuItems }: AppMenuProps) => {
  ...
  const hasPermission = useCallback((key: string): boolean => {
    ...
    const roles = (admin.roles || []).map((r) => r.toLowerCase())
    ...
    if (key === 'end-users') {
      ...
      // GET /admin/end-users is gated by requirePermission('users:read',
      // 'admin:read') on the backend, so this is a real (not cosmetic) check.
      return userPerms.includes('users:read') || userPerms.includes('admin:read')
    }
    if (key === 'roles-permissions') {
      return userPerms.includes('roles:read') || userPerms.includes('permissions:read')
    }
    if (key === 'email-settings') {
      return userPerms.includes('email_branding.read') || userPerms.includes('email_template.read') || userPerms.includes('email_logs.read')
    }
    if (key === 'sms-settings') {
      return (
        userPerms.includes('communication.providers.read') ||
        userPerms.includes('communication.routing.read') ||
        userPerms.includes('communication.templates.read') ||
        userPerms.includes('communication.logs.read')
      )
    }
    if (key === 'api-health') {
      return userPerms.includes('communication.health.read')
    }
    // Phase 2.6A (docs/phase-2-6a-app-aware-communication-routing-ui.md):
    // gated with the same seeded permission keys the corresponding
    // backend routes already require via requirePermission().
    if (key === 'comm-providers') {
      return userPerms.includes('communication.providers.read')
    }
    if (key === 'comm-routing-rules') {
      return userPerms.includes('communication.routing.read')
    }
    if (key === 'comm-provider-health') {
      return userPerms.includes('communication.health.read')
    }
    if (key === 'comm-delivery-logs' || key === 'comm-provider-audit-logs') {
      return userPerms.includes('communication.logs.read')
    }
    if (key === 'comm-app-branding') {
      return userPerms.includes('email_branding.read')
    }
    if (key === 'comm-template-overrides') {
      return userPerms.includes('email_template.read')
    }
    return true
  }, [admin])
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    // Remove titles that have no items following them
