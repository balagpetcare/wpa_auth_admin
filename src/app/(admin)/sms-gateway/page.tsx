...
export default function SmsGatewayPage() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const res = await communicationApi.listSmsProviders({
      ...
      else setError({ message: 'Unable to load SMS gateways.' })
    } finally {
      setLoading(false)
    }
  }

  useEffect(() => { load() }, [countryFilter, environmentFilter, statusFilter, healthFilter])
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center gap-3 flex-wrap mb-4">
        <div>
          <h4 className="fw-bold text-dark mb-1">SMS / OTP Gateway</h4>
          <p className="text-muted mb-0 fs-13">Dedicated SMS provider management.</p>
        </div>
        <div className="d-flex gap-2">
          <Link href="/sms-gateway/new" className="btn btn-primary"><IconifyIcon icon="solar:add-circle-bold-duotone" className="me-1" />New Provider</Link>
        </div>
      </div>
      ...
      <Card className="shadow-sm border-0 mb-3">
        <Card.Body className="d-flex flex-wrap gap-2 align-items-center">
          <Form.Control style={{ maxWidth: 180 }} placeholder="Country code" value={countryFilter} onChange={(e) => setCountryFilter(e.target.value)} />
      ...
      <Card className="shadow-sm border-0">
        <Card.Body className="p-0">
          {loading ? <div className="text-center py-5"><Spinner animation="border" variant="primary" /></div> : providers.length === 0 ? (
            <EmptyState message="No SMS gateways found." icon="solar:chat-square-bold-duotone" />
          ) : (
            <Table hover responsive className="mb-0 align-middle">
              <thead className="table-light">
                <tr>
                  <th className="px-4">Provider</th><th>Country / Global</th><th>Environment</th><th>Status</th><th>Health</th><th>Priority</th><th>Purposes</th><th className="text-end px-4">Actions</th>
                </tr>
              </thead>
              <tbody>
                {providers.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{p.supportedPurposes.map((purpose) => <Badge key={purpose} bg="soft-secondary" className="text-secondary me-1">{purpose}</Badge>)}</td>
                    <td className="px-4">
                      <div className="d-flex justify-content-end flex-wrap gap-2">
                        <Link href={`/sms-gateway/${p.id}`} className="btn btn-link p-0"><IconifyIcon icon="solar:eye-bold-duotone" className="me-1" />View</Link>
                        <Link href={`/sms-gateway/${p.id}/edit`} className="btn btn-link p-0 text-primary"><IconifyIcon icon="solar:pen-bold-duotone" className="me-1" />Edit</Link>
                        <Link href={`/sms-gateway/${p.id}?panel=test`} className="btn btn-link p-0 text-info">
